...
(() => {
  ...
  const DEFAULT_DISTRIBUTION_ITEMS = [
    ...
    "지휘 의사결정-선착대 대응활동 유효성 판단",
    "지휘 의사결정-현장 위험성 판단",
    "지휘 의사결정-핵심목표(대응 지침) 제시",
    "지휘 의사결정-1차 출동대 임무지시 및 조정",
    "지휘 의사결정-추가 자원 요청",
    "지휘 의사결정-차량배치 조정",

    "대응활동-소방활동구역 설정 및 통제",
    "대응활동-소방용수공급체계 구축",
    "대응활동-(단계별)소방력 배치 및 조정",
    "대응활동-현장통합 및 단위지휘관 운영",
    "대응활동-출동대 대기관리",
    "대응활동-대기장소 운영",
    "대응활동-전술상황판 기록",

    "진행상황 관리-진행상황 파악",
    "진행상황 관리-상황 미개선 및 악화시 대응조치",
    "진행상황 관리-우선순위보고 조치",
    "진행상황 관리-초진선언",
    "진행상황 관리-전술우선순위 관리",
    "진행상황 관리-완진절차 준수",
    ...
    "위기관리,리더십-돌발 및 위기상황 대응",
    "위기관리,리더십-스트레스 관리",
    "위기관리,리더십-리더로서의 능숙한 작전 운영",
    "위기관리,리더십-인명구조 목표달성의 적절성",
    "위기관리,리더십-출동대 안전관리의 적절성",
    "위기관리,리더십-시민보호 및 피해최소화 작전의 적절성"
  ];
  ...
  function getPage() {
    return document.querySelector('[data-course-page="중급"]');
  }
  ...
  function updatePreview() {
    ...
    if (preview) {
      preview.textContent = `중급 · ${type} · ${roundLabel(roundNumber)} · ${count}개 생성`;
    }
  }
  ...
  function renderTokenManage() {
    ...
    body.innerHTML = tokens
      .map((token) => `
        <tr>
          <td>
            <input type="checkbox" class="intermediate-token-manage-check" value="${escapeHtml(token.id)}" />
          </td>
          <td>${escapeHtml(token.type)}</td>
          <td>${escapeHtml(roundLabel(token.round))}</td>
          <td>${escapeHtml(token.displayName)}</td>
  ...
  function makeBarValueClass(value, total) {
    ...
    if (pct >= 70) return "inside";
    if (pct >= 40) return "inside-dark";
    return "";
  }
    ...
    return `
      <div class="category-item-bar-row">
        ...
        <div class="category-item-bar-track">
          <div class="category-item-bar-fill ${cls}" style="width:${makeBarWidth(value, total)};"></div>
  ...
  function interpretScoreDistribution(scores, bins) {
    ...
    if (range <= 15 && top1 / total >= 0.5) return "집중형";
    if (nonZeroBins >= 8 && range >= 30) return "분산형";
    if (top1 / total < 0.35 && top2 / total < 0.35 && range >= 40) return "양극화 가능성";
    return "보통";
  }
  ...
  function renderTotalHistogram(summary, type, round) {
    const area = q("#intermediate-total-histogram-area");
    const stats = q("#intermediate-total-score-stats");
    const interpretation = q("#intermediate-total-score-interpretation");
    ...
    area.innerHTML = `
      ...
      <div style="position:relative; width:100%; height:360px;">
        <canvas id="intermediate-total-histogram-canvas"></canvas>
      </div>
    `;
  ...
  function renderItemDistributionChart(rows, type, round) {
    const area = q("#intermediate-item-distribution-chart-area");
    if (!area) return;
  ...
  function renderDistribution() {
    ...
    const itemArea = q("#intermediate-item-distribution-chart-area");
  ...
  function renderDeleteList() {
    ...
    body.innerHTML = submissions
      .map((sub) => `
        <tr>
          <td><input type="checkbox" class="intermediate-delete-check" value="${escapeHtml(sub.id)}" /></td>
  ...
  function renderDetailList() {
    ...
    list.innerHTML = submissions
      .map((sub) => `
        <li class="intermediate-detail-item" data-id="${escapeHtml(sub.id)}">
    ...
    qa(".intermediate-detail-item", getPage()).forEach((item) => {
  ...
  function attachTokenFormEvents() {
    ...
    attachRoundStepper(
      ...
      "#intermediate-token-round-decrease",
      "#intermediate-token-round-increase",
      updatePreview
    );
  ...
  function attachTokenManageEvents() {
    ...
    q("#intermediate-token-delete-selected-btn")?.addEventListener("click", deleteSelectedTokens);
    q("#intermediate-token-delete-all-btn")?.addEventListener("click", deleteAllFilteredTokens);
  }
  ...
  function attachDistributionEvents() {
    ...
    q("#intermediate-download-distribution-btn")?.addEventListener("click", () => {
      ...
      a.href = url;
      a.download = `중급_점수분포_${type}_${roundLabel(round)}.csv`;
      a.click();
  ...
  function attachDeleteEvents() {
    ...
    q("#intermediate-delete-selected-btn")?.addEventListener("click", deleteSelectedSubmissions);
    q("#intermediate-delete-all-btn")?.addEventListener("click", deleteAllFilteredSubmissions);
  }
  ...
  async function initIntermediateAdminPage() {
    const page = getPage();
    if (!page) return;
  ...
  window.initIntermediateAdminPage = initIntermediateAdminPage;
})();
