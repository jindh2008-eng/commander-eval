...
function getTokenFromUrl() {
  ...
  return String(params.get("token") || "").trim().toUpperCase();
}
...
function renderTable(level) {
  ...
  if (!data) {
    sheetTitle.textContent = "평가표를 불러올 수 없습니다.";
                ...
                <div class="score-item">
                  ...
                  <div class="grade-buttons">
                    <label class="grade-option high">
                      <input type="radio" class="item-grade" name="item_${item.index}" value="high" data-score="${item.score}" />
                      <span class="grade-label">상</span>
                    </label>

                    <label class="grade-option mid">
                      <input type="radio" class="item-grade" name="item_${item.index}" value="mid" data-score="${item.score}" />
...
function resolveLevelFromCourse(course) {
...
async function validateTokenAndSetup() {
  ...
  const resolvedLevel = resolveLevelFromCourse(tokenData.course);

  if (!resolvedLevel || !sheetData[resolvedLevel]) {
    tokenInfoEl.textContent = "지원되지 않는 과정 토큰입니다.";
    tokenInfoEl.style.color = "red";
    setUiEnabled(false);
    return;
  }
  ...
  currentTokenDocId = tokenDoc.id;
  currentTokenData = tokenData;
  currentCourse = tokenData.course;
  ...
  tokenInfoEl.textContent =
    `토큰 확인 완료 · ${currentCourse} 평가 진행 가능 · 유형: ${tokenData.type || "-"} / 회차: ${tokenData.round || "-"}회차 / 토큰: ${tokenData.tokenValue}`;
...
submitBtn.addEventListener("click", async () => {
  if (!tokenValidated || isSubmitting || !currentLevel || !currentTokenDocId) return;
  ...
  try {
    await runTransaction(db, async (transaction) => {
      const tokenRef = doc(db, "evaluationTokens", currentTokenDocId);
      const submissionRef = doc(collection(db, "evaluationSubmissions"));

      const tokenDocSnap = await transaction.get(tokenRef);

      if (!tokenDocSnap.exists()) {
        throw new Error("토큰이 존재하지 않습니다.");
      }

      const latestToken = tokenDocSnap.data();
      const latestLevel = resolveLevelFromCourse(latestToken.course);

      if (!latestLevel || !sheetData[latestLevel]) {
        throw new Error("지원되지 않는 과정 토큰입니다.");
      }

      if (latestToken.submitted) {
        throw new Error("이미 제출 완료된 토큰입니다.");
      }

      transaction.set(submissionRef, {
        course: latestToken.course || "",
        type: latestToken.type || "",
        round: latestToken.round || "",
        displayName: latestToken.displayName || "",
        tokenValue: latestToken.tokenValue,
        tokenDocId: currentTokenDocId,
        level: latestLevel,
        levelTitle: sheetData[latestLevel].title,
        totalPossible,
        totalScore,
        comment,
        itemResults,
